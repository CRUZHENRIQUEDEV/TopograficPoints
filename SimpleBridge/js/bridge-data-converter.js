...
function generateGeneralConfigDataFromObra(obra) {
  ...
  const qtdTramos =
    parseInt(obra["QTD TRAMOS"]) || parseInt(obra.QTD_TRAMOS) || 1;
  ...
  const spanLengths = [];
  const comprimentoTramosValue =
    obra["COMPRIMENTO TRAMOS"] || obra.COMPRIMENTO_TRAMOS;
  ...
  if (spanLengths.length === 0) {
    console.warn(
      `  ⚠️ Nenhum tramo definido, distribuindo ${comprimento}m em ${qtdTramos} tramos iguais`
    );
  ...
  const bridgeSpanCollection = createBridgeSpanCollection(
    spanLengths,
    comprimento
  );

  console.log(
    `  ✓ BridgeSpanCollection criado com ${bridgeSpanCollection.Count} tramos`
  );
...
function createBridgeSpanCollection(spanLengths, expectedTotalLength) {
  ...
  const totalLength = roundTo3Decimals(
    spanLengths.reduce((sum, length) => sum + length, 0)
  );
...
function generateBridgeTransitionDataFromObra(obra) {
  ...
  return {
    ...
    SlabAbutmentLength: roundTo3Decimals(
      parseFloat(
        obra["COMPRIMENTO ENCONTRO LAJE"] || obra.COMPRIMENTO_ENCONTRO_LAJE
      ) || 0.0
    ),
    SlabAbutmentLeftOffset: roundTo3Decimals(
      parseFloat(
        obra["DESLOCAMENTO ESQUERDO ENCONTRO LAJE"] ||
          obra.DESLOCAMENTO_ESQUERDO_ENCONTRO_LAJE
      ) || 0.0
    ),
    SlabAbutmentRightOffset: roundTo3Decimals(
      parseFloat(
        obra["DESLOCAMENTO DIREITO ENCONTRO LAJE"] ||
          obra.DESLOCAMENTO_DIREITO_ENCONTRO_LAJE
      ) || 0.0
    ),
    ParallelWing: createZSElementTypeFromValue(
      obra["TIPO ALA PARALELA"] || obra.TIPO_ALA_PARALELA
    ),
    PerpendicularWing: createZSElementTypeFromValue(
      obra["TIPO ALA PERPENDICULAR"] || obra.TIPO_ALA_PERPENDICULAR
    ),
    WingLength: roundTo3Decimals(
      parseFloat(obra["COMPRIMENTO ALA"] || obra.COMPRIMENTO_ALA) || 0.0
    ),
    WingThickness: roundTo3Decimals(
      parseFloat(obra["ESPESSURA ALA"] || obra.ESPESSURA_ALA) || 0.0
    ),
    AbutmentType: createZSElementTypeFromValue(
      obra["TIPO ENCONTRO"] || obra.TIPO_ENCONTRO
    ),
    TransitionSlabType: createZSElementTypeFromValue(
      obra["LAJE TRANSICAO"] || obra.LAJE_TRANSICAO
    ),
    ReinforcedEarthWallType: createZSElementTypeFromValue(
      obra["TIPO TERRA ARMADA"] || obra.TIPO_TERRA_ARMADA
    ),
  };
}
...
function generateSuperstructureDataFromObra(obra) {
  ...
  return {
    ...
    SlabThickness: roundTo3Decimals(
      parseFloat(obra["ESPESSURA LAJE"] || obra.ESPESSURA_LAJE) || 0.0
    ),
    ...
    CornerAngleType: createZSElementTypeFromValue(
      obra["TIPO CANTONEIRA"] || obra.TIPO_CANTONEIRA
    ),
    LeftDisplacement: roundTo3Decimals(
      parseFloat(obra["DESLOCAMENTO ESQUERDO"] || obra.DESLOCAMENTO_ESQUERDO) ||
        0.0
    ),
    RightDisplacement: roundTo3Decimals(
      parseFloat(obra["DESLOCAMENTO DIREITO"] || obra.DESLOCAMENTO_DIREITO) ||
        0.0
    ),
  };
}
...
function generateBridgePierDataFromObra(obra) {
  ...
  const pillarLengths = [];
  const comprimentoPilaresValue =
    obra["COMPRIMENTO PILARES"] || obra.COMPRIMENTO_PILARES;
  ...
  const numberOfPillars =
    parseInt(obra["QTD PILARES"] || obra.QTD_PILARES) || 2;
  const isOffCenter =
    (obra["PILAR DESCENTRALIZADO"] || obra.PILAR_DESCENTRALIZADO) === "Sim";

  // Tipos de elementos (comuns a todos os apoios)
  const pillarType = createZSElementTypeFromValue(
    obra["TIPO PILAR"] || obra.TIPO_PILAR
  );
  const crossbeamType = createZSElementTypeFromValue(
    obra["TIPO TRAVESSA"] || obra.TIPO_TRAVESSA
  );
  const crossbeamHeight = roundTo3Decimals(
    parseFloat(obra["ALTURA TRAVESSA"] || obra.ALTURA_TRAVESSA) || 0.0
  );
  const crossbeamLength = roundTo3Decimals(
    parseFloat(obra["COMPRIMENTO TRAVESSA"] || obra.COMPRIMENTO_TRAVESSA) || 0.0
  );
  const transitionSupportType = createZSElementTypeFromValue(
    obra["TIPO APOIO TRANSICAO"] || obra.TIPO_APOIO_TRANSICAO
  );
  const foundationType = createZSElementTypeFromValue(
    obra["TIPO BLOCO SAPATA"] || obra.TIPO_BLOCO_SAPATA
  );
  const foundationHeight = roundTo3Decimals(
    parseFloat(obra["ALTURA BLOCO SAPATA"] || obra.ALTURA_BLOCO_SAPATA) || 0.0
  );
  const foundationWidth = roundTo3Decimals(
    parseFloat(obra["LARGURA BLOCO SAPATA"] || obra.LARGURA_BLOCO_SAPATA) || 0.0
  );
  const foundationLength = roundTo3Decimals(
    parseFloat(
      obra["COMPRIMENTO BLOCO SAPATA"] || obra.COMPRIMENTO_BLOCO_SAPATA
    ) || 0.0
  );
  const bearingType = createZSElementTypeFromValue(
    obra["TIPO APARELHO APOIO"] || obra.TIPO_APARELHO_APOIO
  );
  ...
  const pillarBracingBeamQuantity =
    parseInt(
      obra["QTD VIGA CONTRAVENTAMENTO PILAR"] ||
        obra.QTD_VIGA_CONTRAVENTAMENTO_PILAR
    ) || 0;
  const pillarBracingBeamHeight = roundTo3Decimals(
    parseFloat(
      obra["ALTURA VIGA CONTRAVENTAMENTO"] || obra.ALTURA_VIGA_CONTRAVENTAMENTO
    ) || 0.0
  );
  const pillarBracingBeamLength = roundTo3Decimals(
    parseFloat(
      obra["COMPRIMENTO VIGA CONTRAVENTAMENTO"] ||
        obra.COMPRIMENTO_VIGA_CONTRAVENTAMENTO
    ) || 0.0
  );
  const connectionBeamType = createZSElementTypeFromValue(
    obra["TIPO VIGA LIGACAO"] || obra.TIPO_VIGA_LIGACAO
  );
  const pillarJacketType = createZSElementTypeFromValue(
    obra["TIPO ENCAMISAMENTO"] || obra.TIPO_ENCAMISAMENTO
  );
  const foundationConnectionBeamType = createZSElementTypeFromValue(
    obra["TIPO LIGACAO FUNDACOES"] || obra.TIPO_LIGACAO_FUNDACOES
  );
  const foundationConnectionBeamHeight = roundTo3Decimals(
    parseFloat(
      obra["ALTURA VIGA LIGACAO FUNDACOES"] ||
        obra.ALTURA_VIGA_LIGACAO_FUNDACOES
    ) || 0.0
  );
  const foundationConnectionBeamLength = roundTo3Decimals(
    parseFloat(
      obra["COMPRIMENTO VIGA LIGACAO FUNDACOES"] ||
        obra.COMPRIMENTO_VIGA_LIGACAO_FUNDACOES
    ) || 0.0
  );
...
function generateBridgeComplementaryDataFromObra(obra) {
  return {
    LeftBarrierType: createZSElementTypeFromValue(
      obra["TIPO BARREIRA ESQUERDA"] || obra.TIPO_BARREIRA_ESQUERDA
    ),
    LeftBarrierWidth: roundTo3Decimals(
      parseFloat(
        obra["LARGURA BARREIRA ESQUERDA"] || obra.LARGURA_BARREIRA_ESQUERDA
      ) || 0.0
    ),
    RightBarrierType: createZSElementTypeFromValue(
      obra["TIPO BARREIRA DIREITA"] || obra.TIPO_BARREIRA_DIREITA
    ),
    RightBarrierWidth: roundTo3Decimals(
      parseFloat(
        obra["LARGURA BARREIRA DIREITA"] || obra.LARGURA_BARREIRA_DIREITA
      ) || 0.0
    ),
    LeftSidewalkType: createZSElementTypeFromValue(
      obra["TIPO CALCADA ESQUERDA"] || obra.TIPO_CALCADA_ESQUERDA
    ),
    LeftSidewalkWidth: roundTo3Decimals(
      parseFloat(
        obra["LARGURA CALCADA ESQUERDA"] || obra.LARGURA_CALCADA_ESQUERDA
      ) || 0.0
    ),
    RightSidewalkType: createZSElementTypeFromValue(
      obra["TIPO CALCADA DIREITA"] || obra.TIPO_CALCADA_DIREITA
    ),
    RightSidewalkWidth: roundTo3Decimals(
      parseFloat(
        obra["LARGURA CALCADA DIREITA"] || obra.LARGURA_CALCADA_DIREITA
      ) || 0.0
    ),
    LeftGuardRailType: createZSElementTypeFromValue(
      obra["GUARDA RODAS ESQUERDO"] || obra.GUARDA_RODAS_ESQUERDO
    ),
    LeftGuardRailWidth: roundTo3Decimals(
      parseFloat(
        obra["LARGURA GUARDA RODAS ESQUERDO"] ||
          obra.LARGURA_GUARDA_RODAS_ESQUERDO
      ) || 0.0
    ),
    RightGuardRail: createZSElementTypeFromValue(
      obra["GUARDA RODAS DIREITO"] || obra.GUARDA_RODAS_DIREITO
    ),
    RightGuardRailWidth: roundTo3Decimals(
      parseFloat(
        obra["LARGURA GUARDA RODAS DIREITO"] ||
          obra.LARGURA_GUARDA_RODAS_DIREITO
      ) || 0.0
    ),
    LeftGuardBodyType: createZSElementTypeFromValue(
      obra["GUARDA CORPO ESQUERDO"] || obra.GUARDA_CORPO_ESQUERDO
    ),
    LeftGuardBodyWidth: roundTo3Decimals(
      parseFloat(
        obra["LARGURA GUARDA CORPO ESQUERDO"] ||
          obra.LARGURA_GUARDA_CORPO_ESQUERDO
      ) || 0.0
    ),
    RightGuardBodyType: createZSElementTypeFromValue(
      obra["GUARDA CORPO DIREITO"] || obra.GUARDA_CORPO_DIREITO
    ),
    RightGuardBodyWidth: roundTo3Decimals(
      parseFloat(
        obra["LARGURA GUARDA CORPO DIREITO"] ||
          obra.LARGURA_GUARDA_CORPO_DIREITO
      ) || 0.0
    ),
    PavementType: createZSElementTypeFromValue(
      obra["TIPO PAVIMENTO"] || obra.TIPO_PAVIMENTO
    ),
    ExpansionJointType: createZSElementTypeFromValue(
      obra["TIPO JUNTA DILATACAO"] || obra.TIPO_JUNTA_DILATACAO
    ),
    CradleType: createZSElementTypeFromValue(
      obra["TIPO BERCO"] || obra.TIPO_BERCO
    ),
    NumberOfBuzinotes:
      parseInt(obra["QTD BUZINOTES"] || obra.QTD_BUZINOTES) || 0,
  };
}
...
window.generateBridgeTransitionDataFromObra =
  generateBridgeTransitionDataFromObra;
...
window.generateBridgeComplementaryDataFromObra =
  generateBridgeComplementaryDataFromObra;
